...
const SkillsPassportSkeleton = () => {
    return (
        <div className="max-w-5xl mx-auto animate-pulse">
            ...
            {/* Tabs Skeleton */}
            <div className="flex gap-2 mb-8 bg-slate-50 dark:bg-slate-800/50 p-1.5 rounded-xl shadow-sm border border-slate-100 dark:border-slate-800 w-fit">
                {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="h-9 w-24 bg-slate-200 dark:bg-slate-700 rounded-lg" />
                ))}
            </div>
            ...
            <div className="bg-white dark:bg-[#1e293b] rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                ...
                <div className="p-6 md:p-8 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
                    ...
                    <div className="text-right">
                        <div className="h-8 w-16 bg-slate-200 dark:bg-slate-700 rounded mb-1 ml-auto" />
                        <div className="h-3 w-12 bg-slate-200 dark:bg-slate-700 rounded ml-auto" />
                    </div>
                </div>

                <div className="grid lg:grid-cols-5 bg-white dark:bg-[#1e293b]">
                    {/* Left: Radar Chart Skeleton */}
                    <div className="lg:col-span-2 p-6 md:p-8 border-b lg:border-b-0 lg:border-r border-slate-100 dark:border-slate-700 flex flex-col items-center justify-center bg-slate-50/30 dark:bg-slate-800/20">
                        <div className="w-64 h-64 rounded-full bg-slate-200 dark:bg-slate-700" />
                        <div className="mt-6 h-4 w-32 bg-slate-200 dark:bg-slate-700 rounded" />
                    </div>
