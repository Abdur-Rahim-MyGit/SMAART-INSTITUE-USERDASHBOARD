...
const SimpleInstitutionSelector = ({ onSelect }) => {
  ...
  return (
    ...
    >
      <div className="bg-gradient-to-br from-teal-400 via-teal-500 to-teal-600 rounded-2xl p-4 sm:p-6 md:p-8 border border-teal-300 shadow-glow">
        <h3 className="text-white text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-center drop-shadow-lg">
          Select Your Institution
        </h3>
        
        <div className="relative">
          <div 
            className="w-full h-12 sm:h-14 md:h-16 text-sm sm:text-base md:text-lg bg-white/90 border-2 border-teal-300 text-gray-800 hover:border-teal-400 hover:bg-white transition-all duration-300 rounded-xl font-medium shadow-lg cursor-pointer flex items-center px-3 sm:px-4"
            onClick={() => setShowDropdown(!showDropdown)}
          >
            {selectedInstitution ? (
              <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                <Building2 className="h-4 w-4 sm:h-5 sm:w-5 text-teal-600 flex-shrink-0" />
                <div className="text-left min-w-0">
                  ...
                  <div className="text-xs sm:text-sm text-gray-600 flex items-center gap-1">
                    ...
                    <span className="ml-1 sm:ml-2 text-xs bg-gray-200 px-1 sm:px-2 py-0.5 sm:py-1 rounded flex-shrink-0">
          ...
          {showDropdown && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-teal-300 rounded-xl shadow-2xl z-50 max-h-80 overflow-hidden">
                ...
                {loading ? (
                  ...
                  colleges.map((college) => (
                    <div
                      ...
                      className="flex items-center gap-3 p-4 cursor-pointer hover:bg-teal-50 transition-colors duration-200 border-b border-gray-100 last:border-b-0"
                    >
