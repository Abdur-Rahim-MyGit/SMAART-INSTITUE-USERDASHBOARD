...
import { generateBig5Report } from "@/utils/pdfGenerator";
...
const BigFiveTest = () => {
  ...
  const [big5Results, setBig5Results] = useState(null);
  ...
  useEffect(() => {
    const initializeAssessment = async () => {
      try {
        ...
        // Fetch Big Five assessment by code (more reliable than description)
        ...
        if (userResultsResponse.success && userResultsResponse.data) {
          // Check if any completed result has assessmentCode "ASM00001"
          const hasCompletedBig5 = userResultsResponse.data.some(
            result => result.assessmentCode === "ASM00001" && result.completionStatus === "completed"
          );

          if (hasCompletedBig5) {
            // User has already completed the Big Five assessment
            setError("You have already completed the Big Five assessment. You can only take it once.");
            setLoading(false);
            return;
          }
        }
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (response.success) {
        setSubmitted(true);
        // Store Big Five results in state and sessionStorage
        setBig5Results(response.data.scores);
        sessionStorage.setItem("big5Results", JSON.stringify(response.data));
      }
    } catch (err) {
                ...
                <div className="p-4 sm:p-6">
                  {!submitted ? (
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-6 sm:py-8">
                      <h3 className="text-2xl sm:text-3xl font-bold text-[#002147] mb-4 text-center">Your Big Five Results</h3>
                      <p className="text-sm sm:text-base text-gray-600 mb-6 text-center max-w-2xl mx-auto">
                        Here are your personality trait scores based on the Big Five model.
                      </p>
                        ...
                        <div className="space-y-6 max-w-4xl mx-auto">
                          ...
                          <div className="bg-white rounded-xl p-6 border border-[#daa520] shadow-sm">
                            <div className="flex items-center justify-between mb-4">
                              <h4 className="text-lg font-semibold text-[#002147] text-center flex-1">Personality Profile</h4>
                              <button
                                onClick={async () => {
                                  ...
                                  try {
                                    ...
                                    if (data.success && data.data) {
                                      const studentData = {
                                        ...userData,
                                        collegeName: data.data.college?.name || 'N/A'
                                      };
                                      generateBig5Report(big5Results, studentData);
                                    } else {
                                      generateBig5Report(big5Results, userData);
                                    }
                                  } catch (error) {
                                ...
                                className="flex items-center gap-2 px-4 py-2 bg-[#30919D] text-white rounded-lg hover:bg-[#277a84] transition-all shadow-sm text-sm font-medium"
                              >
                                <Download className="w-4 h-4" />
                                Download Report
                              </button>
                            </div>
                            <Big5RadarChart scores={big5Results} />
                          </div>

                          {/* Trait Cards */}
                          <div className="space-y-4">
                          {Object.entries(big5Results).map(([trait, data]) => {
                            const traitName = trait.charAt(0).toUpperCase() + trait.slice(1);
                            const levelColor = data.level === 'High' ? 'bg-[#30919D]' : data.level === 'Moderate' ? 'bg-[#daa520]' : 'bg-gray-400';
                            const levelTextColor = data.level === 'High' ? 'text-white' : data.level === 'Moderate' ? 'text-[#002147]' : 'text-gray-700';
                            const levelBgColor = data.level === 'High' ? 'bg-[#30919D]/20' : data.level === 'Moderate' ? 'bg-[#daa520]/20' : 'bg-gray-200';
                            const percentage = data.raw;
                            ...
                            return (
                              <motion.div
                                ...
                                transition={{ delay: Object.keys(big5Results).indexOf(trait) * 0.1 }}
                                className="bg-white rounded-xl p-4 sm:p-6 border border-[#daa520] shadow-sm"
                              >
                                <div className="flex items-center justify-between mb-3">
                                  <h4 className="text-lg sm:text-xl font-bold text-[#002147]">{traitName}</h4>
                                  <div className="flex items-center gap-3">
                                    <span className="text-2xl font-bold text-[#30919D]">{data.raw}</span>
                                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${levelBgColor} ${levelTextColor}`}>
                                      {data.level}
                                    </span>
                                  </div>
                                </div>
                                <div className="h-3 bg-gray-200 rounded-full overflow-hidden">
                                  <motion.div
                                    ...
                                    transition={{ duration: 0.8, delay: Object.keys(big5Results).indexOf(trait) * 0.1 + 0.2 }}
                      ...
                      <div className="mt-8 flex items-center justify-center gap-3 px-4">
                        <a href="/dashboard/assessments" className="px-6 py-3 text-sm sm:text-base rounded-lg bg-[#30919D] text-white font-semibold hover:bg-[#277a84] transition-all shadow-md">Back to Assessments</a>
                      </div>
                    </motion.div>
                  )}
                </div>
              ...
              {!submitted && (
                <div className="lg:w-80 bg-white rounded-xl lg:rounded-2xl border border-[#daa520] shadow-lg p-4 sm:p-6">
                  ...
                  <div className="mt-6 pt-4 border-t border-gray-200">
                    <button
                      onClick={async () => {
                        // Developer bypass - auto-answer all questions and submit
                        if (!resultId) return;
                        
                        try {
                          setSubmitting(true);
                          
                          // Auto-save random answers for all unanswered questions
                          for (const q of questions) {
                            if (!selectedAnswers[q._id]) {
                              const randomAnswer = Math.floor(Math.random() * 5) + 1;
                              await assessmentApi.saveAnswer(resultId, q._id, randomAnswer, q.questionText);
                            }
                          }
                      ...
                      disabled={submitting}
                      className="w-full text-[10px] text-white/60 hover:text-white bg-gray-800 hover:bg-gray-700 py-2 px-3 rounded-md uppercase tracking-widest font-bold transition-colors disabled:opacity-50"
                    >
                      {submitting ? 'Skipping...' : '⚡ DEV SKIP'}
                    </button>
                  </div>
                </div>
              )}
