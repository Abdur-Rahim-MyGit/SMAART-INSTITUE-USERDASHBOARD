...
const ARQTest = () => {
  ...
  const [arqResults, setArqResults] = useState(null);
  ...
  useEffect(() => {
    const initializeAssessment = async () => {
      try {
        ...
        // Fetch ARQ assessment by code
        const assessmentResponse = await assessmentApi.getByCode("ASM00005");
        ...
        if (userResultsResponse.success && userResultsResponse.data) {
          // Check if any completed result has assessmentCode "ASM00005"
          const hasCompletedARQ = userResultsResponse.data.some(
            result => result.assessmentCode === "ASM00005" && result.completionStatus === "completed"
          );

          if (hasCompletedARQ) {
            // User has already completed the ARQ assessment
            setError("You have already completed the Adaptability and Resilience Quotient assessment. You can only take it once.");
            setLoading(false);
            return;
          }
        }
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (response.success) {
        setSubmitted(true);
        // Store ARQ results in state
        if (response.data.compositeScore !== undefined) {
          setArqResults({
            adaptabilityScore: response.data.adaptabilityScore,
        ...
        sessionStorage.setItem("arqResults", JSON.stringify(response.data));
      }
    } catch (err) {
              ...
              <div className="flex-1 bg-white rounded-xl lg:rounded-2xl border border-[#daa520] shadow-lg overflow-hidden">
                ...
                <div className="px-4 sm:px-6 pt-4 sm:pt-6 pb-2">
                  <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-[#002147] text-center leading-tight">
                    Adaptability and resilience turn challenges into opportunities for growth
                  </h2>
                </div>
                      ...
                      {arqResults ? (
                        <>
                          <div className="text-center mb-8">
                            ...
                            <h3 className="text-2xl sm:text-3xl font-bold text-[#002147] mb-2">Your Adaptability & Resilience Result</h3>
                            <p className="text-sm sm:text-base text-gray-600 mb-6 max-w-2xl mx-auto">
                              Your adaptability and resilience have been assessed based on 15 key indicators.
                            </p>
                          </div>
                          ...
                          <div className="max-w-3xl mx-auto mb-8 space-y-4">
                            <div className="bg-white rounded-xl p-6 border border-[#daa520] shadow-sm">
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                                <div className="text-center">
                                  <div className="text-sm text-gray-600 mb-1">Adaptability Score</div>
                                  <div className="text-3xl font-bold text-[#30919D]">{Math.round(arqResults.adaptabilityScore)}</div>
                                  <div className="text-xs text-gray-500">9 Questions</div>
                                </div>
                                <div className="text-center">
                                  <div className="text-sm text-gray-600 mb-1">Resilience Score</div>
                                  <div className="text-3xl font-bold text-[#daa520]">{Math.round(arqResults.resilienceScore)}</div>
                                  <div className="text-xs text-gray-500">6 Questions</div>
                                </div>
                                <div className="text-center">
                                  <div className="text-sm text-gray-600 mb-1">Composite ARQ</div>
                                  <div className="text-3xl font-bold text-[#002147]">{Math.round(arqResults.compositeScore)}</div>
                                  <div className="text-xs text-gray-500">Average Score</div>
                                </div>
                              </div>

                              <h4 className="text-lg font-semibold text-[#002147] mb-4 text-center">ARQ Level</h4>
                              ...
                              <div className="relative h-12 bg-gray-200 rounded-full overflow-hidden mb-4">
                                <div 
                                  ...
                                  style={{ 
                                    width: `${arqResults.compositeScore}%`,
                                    backgroundColor: arqResults.colorCode === 'red' ? '#EF4444' :
                                                   arqResults.colorCode === 'amber' ? '#daa520' :
                                                   arqResults.colorCode === 'green' ? '#30919D' : '#30919D'
                                  }}
                                />
                              </div>
                        ...
                        <>
                          ...
                          <p className="text-sm sm:text-base text-gray-600 mb-6 max-w-2xl mx-auto text-center">
                            Thank you for completing the Adaptability and Resilience Quotient assessment. Your responses have been recorded.
                          </p>
