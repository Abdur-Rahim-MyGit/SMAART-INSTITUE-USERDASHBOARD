...
const AIQTest = () => {
  ...
  const [aiqResults, setAiqResults] = useState(null);
  ...
  useEffect(() => {
    const initializeAssessment = async () => {
      try {
        ...
        // Fetch AIQ assessment by code
        const assessmentResponse = await assessmentApi.getByCode("ASM00006");
        ...
        if (userResultsResponse.success && userResultsResponse.data) {
          // Check if any completed result has assessmentCode "ASM00006"
          const hasCompletedAIQ = userResultsResponse.data.some(
            result => result.assessmentCode === "ASM00006" && result.completionStatus === "completed"
          );

          if (hasCompletedAIQ) {
            // User has already completed the AIQ assessment
            setError("You have already completed the Artificial Intelligence Quotient assessment. You can only take it once.");
            setLoading(false);
            return;
          }
        }
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (response.success) {
        setSubmitted(true);
        // Store AIQ results in state
        if (response.data.aiqPercentage !== undefined) {
          setAiqResults({
            subscores: response.data.subscores,
            meanScore: response.data.meanScore,
            aiqPercentage: response.data.aiqPercentage,
        ...
        sessionStorage.setItem("aiqResults", JSON.stringify(response.data));
      }
    } catch (err) {
              ...
              <div className="flex-1 bg-white rounded-xl lg:rounded-2xl border border-[#daa520] shadow-lg overflow-hidden">
                ...
                <div className="px-4 sm:px-6 pt-4 sm:pt-6 pb-2">
                  <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-[#002147] text-center leading-tight">
                    Artificial Intelligence Quotient - Measure Your AI Literacy
                  </h2>
                </div>
                      ...
                      {aiqResults ? (
                        <>
                          <div className="text-center mb-8">
                            ...
                            <h3 className="text-2xl sm:text-3xl font-bold text-[#002147] mb-2">Your AI Literacy Result</h3>
                            <p className="text-sm sm:text-base text-gray-600 mb-6 max-w-2xl mx-auto">
                              Your AI literacy has been assessed across 5 key competency areas.
                            </p>
                          </div>
                          ...
                          <div className="max-w-3xl mx-auto mb-8 space-y-4">
                            <div className="bg-white rounded-xl p-6 border border-[#daa520] shadow-sm">
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
                                <div className="text-center">
                                  <div className="text-xs text-gray-600 mb-1">AI Knowledge</div>
                                  <div className="text-2xl font-bold text-[#30919D]">{Math.round(aiqResults.subscores.a1)}</div>
                                  <div className="text-[10px] text-gray-500">A1</div>
                                </div>
                                <div className="text-center">
                                  <div className="text-xs text-gray-600 mb-1">AI Use & Skills</div>
                                  <div className="text-2xl font-bold text-[#30919D]">{Math.round(aiqResults.subscores.a2)}</div>
                                  <div className="text-[10px] text-gray-500">A2</div>
                                </div>
                                <div className="text-center">
                                  <div className="text-xs text-gray-600 mb-1">Critical Thinking</div>
                                  <div className="text-2xl font-bold text-[#30919D]">{Math.round(aiqResults.subscores.a3)}</div>
                                  <div className="text-[10px] text-gray-500">A3</div>
                                </div>
                                <div className="text-center">
                                  <div className="text-xs text-gray-600 mb-1">AI Ethics</div>
                                  <div className="text-2xl font-bold text-[#30919D]">{Math.round(aiqResults.subscores.a4)}</div>
                                  <div className="text-[10px] text-gray-500">A4</div>
                                </div>
                                <div className="text-center">
                                  <div className="text-xs text-gray-600 mb-1">Self-Efficacy</div>
                                  <div className="text-2xl font-bold text-[#30919D]">{Math.round(aiqResults.subscores.a5)}</div>
                                  <div className="text-[10px] text-gray-500">A5</div>
                                </div>
                              </div>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 border-t pt-6">
                                <div className="text-center">
                                  ...
                                  <div className="text-3xl font-bold text-[#daa520]">{Math.round(aiqResults.meanScore)}</div>
                                  <div className="text-xs text-gray-500">Average of 5 Areas</div>
                                </div>
                                <div className="text-center">
                                  <div className="text-sm text-gray-600 mb-1">AIQ Percentage</div>
                                  <div className="text-3xl font-bold text-[#002147]">{Math.round(aiqResults.aiqPercentage)}%</div>
                                  <div className="text-xs text-gray-500">AI Literacy Score</div>
                                </div>
                              </div>

                              <h4 className="text-lg font-semibold text-[#002147] mb-4 text-center">AIQ Level</h4>
                              ...
                              <div className="relative h-12 bg-gray-200 rounded-full overflow-hidden mb-4">
                                <div 
                                  ...
                                  style={{ 
                                    width: `${aiqResults.aiqPercentage}%`,
                                    backgroundColor: aiqResults.colorCode
                                  }}
                                />
                              </div>
                        ...
                        <>
                          ...
                          <p className="text-sm sm:text-base text-gray-600 mb-6 max-w-2xl mx-auto text-center">
                            Thank you for completing the Artificial Intelligence Quotient assessment. Your responses have been recorded.
                          </p>
