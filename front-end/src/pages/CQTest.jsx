...
const CQTest = () => {
  ...
  const [cqResults, setCqResults] = useState(null);
  ...
  useEffect(() => {
    const initializeAssessment = async () => {
      try {
        ...
        // Fetch CQ assessment
        const assessmentResponse = await assessmentApi.getByCode("ASM00004");
        ...
        if (userResultsResponse.success && userResultsResponse.data) {
          // Check if any completed result has assessmentCode "ASM00004"
          const hasCompletedCQ = userResultsResponse.data.some(
            result => result.assessmentCode === "ASM00004" && result.completionStatus === "completed"
          );

          if (hasCompletedCQ) {
            // User has already completed the CQ assessment
            setError("You have already completed the Creativity Quotient assessment. You can only take it once.");
            setLoading(false);
            return;
          }
        }
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (response.success) {
        setSubmitted(true);
        // Store CQ results in state
        if (response.data.compositeScore !== undefined) {
          setCqResults({
            opennessScore: response.data.opennessScore,
        ...
        sessionStorage.setItem("cqResults", JSON.stringify(response.data));
      }
    } catch (err) {
              ...
              <div className="flex-1 bg-white rounded-xl lg:rounded-2xl border border-[#daa520] shadow-lg overflow-hidden">
                ...
                <div className="px-4 sm:px-6 pt-4 sm:pt-6 pb-2">
                  <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-[#002147] text-center leading-tight">
                    Creativity thrives when you embrace curiosity and think beyond the obvious
                  </h2>
                </div>
                      ...
                      {cqResults ? (
                        <>
                          <div className="text-center mb-8">
                            ...
                            <h3 className="text-2xl sm:text-3xl font-bold text-[#002147] mb-2">Your Creativity Quotient Result</h3>
                            <p className="text-sm sm:text-base text-gray-600 mb-6 max-w-2xl mx-auto">
                              Your creativity has been assessed based on your openness to experience and creative thinking abilities.
                            </p>
                          </div>
                          ...
                          <div className="max-w-3xl mx-auto mb-8 space-y-4">
                            <div className="bg-white rounded-xl p-6 border border-[#daa520] shadow-sm">
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                                <div className="text-center">
                                  <div className="text-sm text-gray-600 mb-1">Openness Score</div>
                                  <div className="text-3xl font-bold text-[#30919D]">{Math.round(cqResults.opennessScore)}</div>
                                  <div className="text-xs text-gray-500">From Big 5</div>
                                </div>
                                <div className="text-center">
                                  <div className="text-sm text-gray-600 mb-1">Creativity Score</div>
                                  <div className="text-3xl font-bold text-[#daa520]">{Math.round(cqResults.creativityScore)}</div>
                                  <div className="text-xs text-gray-500">From Questions</div>
                                </div>
                                <div className="text-center">
                                  <div className="text-sm text-gray-600 mb-1">Composite CQ</div>
                                  <div className="text-3xl font-bold text-[#002147]">{Math.round(cqResults.compositeScore)}</div>
                                  <div className="text-xs text-gray-500">Average Score</div>
                                </div>
                              </div>

                              <h4 className="text-lg font-semibold text-[#002147] mb-4 text-center">Creativity Level</h4>
                              ...
                              <div className="relative h-12 bg-gray-200 rounded-full overflow-hidden mb-4">
                                <div 
                                  ...
                                  style={{ 
                                    width: `${cqResults.compositeScore}%`,
                                    backgroundColor: cqResults.colorCode === 'red' ? '#EF4444' :
                                                   cqResults.colorCode === 'amber' ? '#daa520' :
                                                   cqResults.colorCode === 'green' ? '#30919D' : '#30919D'
                                  }}
                                />
                              </div>
                        ...
                        <>
                          ...
                          <p className="text-sm sm:text-base text-gray-600 mb-6 max-w-2xl mx-auto text-center">
                            Thank you for completing the Creativity Quotient assessment. Your responses have been recorded.
                          </p>
