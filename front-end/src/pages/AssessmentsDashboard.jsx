        ...
        <div className="bg-transparent transition-colors duration-300 min-h-screen pb-8">
            <main>
                <div className="mx-auto max-w-7xl space-y-6 p-8">
                    ...
                    >
                        <div className="flex flex-col sm:flex-row gap-4 sm:items-center xl:items-start flex-1">
                            <button
                                onClick={() => navigate("/dashboard")}
                                className="group flex items-center gap-2 px-3 py-2 text-[#112b6b] dark:text-slate-300 text-xs font-bold uppercase tracking-wider hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-xl transition-all border border-transparent hover:border-slate-200 dark:hover:border-slate-700 w-fit shrink-0 mt-1"
                            >
                                <ArrowLeft stroke={2} className="h-4 w-4 group-hover:-translate-x-1 transition-transform" />
                                {t("my_courses_page.back_to_dashboard", "Back")}
                            </button>
                            <div className="hidden sm:block w-px h-12 bg-slate-200 dark:bg-white/10 shrink-0"></div>
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 xl:border-l border-slate-200 dark:border-white/10 xl:pl-6 shrink-0">
                            {/* Metric Badges */}
                            <div className="flex items-center gap-3">
                                {METRICS.map((metric, idx) => (
                                    <div key={idx} className="flex flex-col bg-slate-50 dark:bg-slate-800/50 px-3 py-1.5 rounded-lg border border-slate-100 dark:border-white/5">
                                        <div className="flex items-center gap-1.5 text-[#1a3884] dark:text-blue-400 mb-0.5">
                                            <metric.icon stroke={2} className="w-3.5 h-3.5" />
                                            <span className="text-sm font-black leading-none">{metric.value}</span>
                                        </div>
