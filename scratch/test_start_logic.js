...
const Result = require('../back-end/models/Result');
const Assessment = require('../back-end/models/Assessment');
...
async function simulate() {
  ...
  try {
    ...
    // 1. Find user (student)
    const user = await db.collection('users').findOne({ email: 'teststudent@gmail.com' });
    if (!user) {
      console.error('Test user teststudent@gmail.com not found!');
      process.exit(1);
    }
