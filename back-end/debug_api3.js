...
async function run() {
    try {
        ...
        const user = await db.collection('students').findOne({_id: studentId}) || await db.collection('users').findOne({_id: studentId});
        ...
        const targetApp = apps.find(a => a._id === '6a6ae6dd6fd4780ecd58386a');
        if (targetApp) {
